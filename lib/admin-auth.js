...
async function fallbackLogin(password, totp) {
  ...
  let totpResult;
  try {
    totpResult = getTotpVerifier()({ token: String(totp || ""), secret: expectedTotpSecret });
  } catch (_error) {
    throw new Error("IDEAS_ADMIN_TOTP_SECRET is invalid. Replace it with the Base32 secret generated for the authenticator app.");
  }

...
function getTotpVerifier() {
  return require("otplib").verifySync;
}
...
async function fallbackLogin(password, totp) {
  ...
  let totpResult;
  try {
    totpResult = getTotpVerifier()({
      token: String(totp || "").trim(),
      secret: expectedTotpSecret.toUpperCase()
    });
  } catch (error) {
    throw new Error(`IDEAS_ADMIN_TOTP_SECRET is invalid (${describeTotpSecret(expectedTotpSecret, error)}). Replace it with the Base32 secret generated for the authenticator app.`);
  }
  const totpMatches = Boolean(totpResult && totpResult.valid);

...
function generateTotpToken(secretBytes, counter) {
...
function verifyTotpToken(token, secret) {
  ...
  if (!new RegExp(`^\\d{${TOTP_DIGITS}}$`).test(normalizedToken)) return false;
  const secretBytes = decodeBase32Secret(secret);
  if (secretBytes.length < 16 || secretBytes.length > 64) {
    throw new Error(`Secret byte length ${secretBytes.length} is outside the allowed range.`);
  }
  const counter = Math.floor(Date.now() / 1000 / TOTP_STEP_SECONDS);
  const expected = generateTotpToken(secretBytes, counter);
  return crypto.timingSafeEqual(Buffer.from(normalizedToken), Buffer.from(expected));
}

function describeTotpSecret(secret, error) {
  return [
    `length=${secret.length}`,
    `base32=${/^[A-Z2-7]+=*$/i.test(secret)}`,
...
async function beginPasskeyAuth(session, context = {}) {
  const { generateAuthenticationOptions } = getWebAuthnServer();
...
async function finishPasskeyAuth({ challengeId, response, session, context }) {
  const { verifyAuthenticationResponse } = getWebAuthnServer();
  const challengeRecord = await store.getdel(`webauthn:${challengeId}`);
  if (!challengeRecord) {
    throw new Error("Passkey challenge expired. Please try again.");
  }
  const storedContext = challengeRecord.context || {};
  const isStepUp = Boolean(challengeRecord.sessionId);
  if (isStepUp && (!session || challengeRecord.sessionId !== session.id)) {
    throw new Error("Passkey challenge does not match this admin session.");
  }
  if (hashPayload(storedContext) !== hashPayload(context || {})) {
    throw new Error("Passkey challenge does not match this request.");
  }
  const passkeys = getConfiguredPasskeys();
  ...
  if (isStepUp && storedContext.action) {
    ...
    const contextHash = hashPayload({
      ...
      route: storedContext.route,
      target: storedContext.target,
      payloadHash: storedContext.payloadHash,
      nonce: storedContext.nonce
    });
...
async function fallbackLogin(password, totp) {
  ...
  const expectedTotpSecret = String(process.env.IDEAS_ADMIN_TOTP_SECRET || "").trim();
  if (!expectedPassword || !expectedTotpSecret) {
    throw new Error("Fallback auth is not configured.");
  }
  if (!/^[A-Z2-7]+=*$/i.test(expectedTotpSecret)) {
    throw new Error("IDEAS_ADMIN_TOTP_SECRET must be the Base32 secret only, with no spaces or otpauth URL.");
  }
  ...
  let totpResult;
  try {
    totpResult = verifyTotpToken(totp, expectedTotpSecret.toUpperCase());
  } catch (error) {
    const message = String(error?.message || "Unknown verifier error").replace(/[^\w .,:;()=-]/g, "").slice(0, 160);
    throw new Error(`IDEAS_ADMIN_TOTP_SECRET is invalid (${describeTotpSecret(expectedTotpSecret, error)}, message=${message}). Replace it with the Base32 secret generated for the authenticator app.`);
  }
  const totpMatches = Boolean(totpResult);

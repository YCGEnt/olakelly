...
function isAllowedPreviewOrigin(origin) {
  try {
    ...
    return url.protocol === "https:"
      && /^olakelly-[a-z0-9-]+-ycgents-projects\.vercel\.app$/i.test(url.hostname);
  } catch (_error) {
    return false;
  }
}
...
function validateOrigin(req, { requireOrigin = true } = {}) {
  ...
  if (!origin || (!getAllowedOrigins().includes(origin) && !isAllowedPreviewOrigin(origin))) {
...
async function consumeNonce(nonce, sessionId) {
  ...
  const record = await store.getdel(key);
...
async function requireStepUp(session, context) {
  ...
  const grant = await store.getdel(key);
  if (!grant) {

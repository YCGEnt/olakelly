...
function isControlledContentBranch(branch) {
  return /^content\/\d{4}-\d{2}-\d{2}\/[a-z0-9-]+$/i.test(String(branch || ""));
}
...
async function deleteBranch(branch) {
  if (!isControlledContentBranch(branch)) {
...
async function createOrUpdatePr({ branch, title, slug, id, payloadHash, summary }) {
  ...
  if (existing) {
    if (existing.head.ref !== branch || !isControlledContentBranch(branch)) {
      throw new Error("Existing publish PR is not attached to the controlled content branch.");
    }

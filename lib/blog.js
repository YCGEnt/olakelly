...
function computePost(input, body) {
  ...
  return {
    ...
    homepage_featured: Boolean(input.homepage_featured),
    homepage_order: Number.isFinite(Number(input.homepage_order)) ? Number(input.homepage_order) : null,
    date: publishedAt,
    updated: updatedAt,
...
function createLatestPostsJson(posts) {
  const featuredPosts = posts
    .filter((post) => post.homepage_featured)
    .sort((left, right) => {
      const leftHasOrder = Number.isFinite(left.homepage_order);
      const rightHasOrder = Number.isFinite(right.homepage_order);

      if (leftHasOrder && rightHasOrder) {
        return left.homepage_order - right.homepage_order;
      }
